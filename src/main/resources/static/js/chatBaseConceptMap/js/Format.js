...
ArrangePanel.prototype.init = function () {
    ...
    // }
    this.container.appendChild(this.addGeneralInfo(this.createPanel()));
    // this.container.appendChild(this.addRelatedIcons(this.createPanel()));
    // this.container.appendChild(this.addElementInfo(this.createPanel()));
};

ArrangePanel.prototype.addGeneralInfo = function(div){
    var ui = this.editorUi;
    var graph = ui.editor.graph;
    var ss = graph.getSelectionCell();
    // console.log(ss.id);
    ...
    //查找图标信息
    var geoIcon;
    // for (let i = 0; i < GeoIconList.length; i++) {
    //     if (GeoIconList[i].id == ss.iconId){
    //         geoIcon = GeoIconList[i];
    //         geoIcon.name = "aa";
    //
    //         console.log(GeoIconList[i].name);
    //     }
    // }
    ...
    //名称
    {
        ...
        nameInput.setAttribute("list","eleList");
        ...
        if (GeoElements != null) {
            ...
            for (let i = 0; i < GeoElements.processes.length; i++) {
                var eles = GeoElements.processes[i].elements;
    ...
    //描述
    {
        ...
        if (geoIcon != null){
            descInput.value = geoIcon.description;
        }
...
ArrangePanel.prototype.addRelatedIcons = function(div){
    ...
    $.ajax({
        ...
        async:true,
        success:  (result) => {
            var mapList = result.relatedConceptMaps;
            var classes = [];
            for (let i = 0; i < mapList.length; i++) {
                classes.push(mapList[i].mapClass);
            }
            classes = Array.from(new Set(classes));

            for (var j = 0;j < classes.length;j++) {
                var cla = this.createTitle(classes[j]);
                div.appendChild(cla);

                var divFold = document.createElement("div");
                for (var i = 0; i < mapList.length; i++) {
                    var pathUrl = mapList[i].pathUrl
                    var name = mapList[i].name;
                    var xml = mapList[i].xml;
                    var belong = mapList[i].mapClass;
                    if(classes[j] === belong){
                        var icon = ui.sidebar.createVertexTemplate('image;html=1;labelBackgroundColor=#ffffff;image='  + pathUrl,
                            ui.sidebar.defaultImageWidth, ui.sidebar.defaultImageHeight, xml, name, null, null, null);
                        divFold.appendChild(icon);
                    }
                };
                ...
                this.addFoldingHandler(cla, divFold);
                //ui.sidebar.addPaletteFunctions2(iconId, classsify[j], false, fns);
            }


            var iconList = result.relatedGeoIcons;
            var classes = [];
            ...
            for (var j = 0;j < classes.length;j++) {
                ...
                for (var i = 0; i < iconList.length; i++) {
                    var pathUrl = iconList[i].pathUrl
 ...
 *
 */
ArrangePanel.prototype.addElementInfo = function (div) {
    var ui = this.editorUi;
    var graph = ui.editor.graph;
    var ss = graph.getSelectionCell();
    console.log(ss);
    ...
    // //spaceInfo input
    // var spaceText = document.createElement('input');
    // // shapeInfo input
    // var shapeText = document.createElement('input');
    ...
    //   }
    //   div.appendChild(ul);
    //   this.addEnrichTextArea(div, 'Description');
    //
    // }
    // else if (ss.image || !cell.isEdge()) {
    //   //url property
    //   this.addEnrichInfoDiv(div, 'URL', urlText);
    //   if (ss.url) {
    //     urlText.value = ss.url;
    //   }
    //   //time property
    //   this.addEnrichInfoDiv(div, 'time property', timeText);
    //   if (ss.timeInfo) {
    //     timeText.value = ss.timeInfo;
    //   }
    //   //space property
    //   this.addEnrichInfoDiv(div, 'space property', spaceText);
    //   if (ss.spaceInfo) {
    //     spaceText.value = ss.spaceInfo;
    //   }
    //   //outlook property
    //   this.addEnrichInfoDiv(div, 'shape property', shapeText);
    //   if (ss.shapeInfo) {
    //     shapeText.value = ss.shapeInfo;
    //   }
    // }
    //
    ...
    //
    //     // text's value ---->   store in conceptItem
    //     conceptItem.url = urlText.value;
    //     conceptItem.timeInfo = timeText.value;
    //     conceptItem.spaceInfo = spaceText.value;
    //     conceptItem.shapeInfo = shapeText.value;
    //
    //
    //     // store in conceptItem-- -- > conceptItem to ss[property]
    //     ss.url = conceptItem.url;
    //     ss.timeInfo = conceptItem.timeInfo;
    //     ss.spaceInfo = conceptItem.spaceInfo;
    //     ss.shapeInfo = conceptItem.shapeInfo;
    //     ss.relation = document.getElementById("relations").value;
    //     console.log("ss的值是" + ss.relation);
    //     var Localizations = [];
    ...
    //几何形状
    {
        ...
        var firstOption = document.createElement("option");
        firstOption.text = "---------- select -----------";
        shapeSelect.appendChild(firstOption);

        for (var i = 0; i < this.shapeArray.length; i++) {
            var option = document.createElement('option');
            option.text = this.shapeArray[i];
            option.style.fontFamily = 'Sans-serif';
            shapeSelect.appendChild(option);
        }

        shapeSelect.style.fontSize = '12px';
        shapeSelect.style.overflow = 'hidden';
        shapeSelect.style.boxSizing = 'border-box';
        shapeSelect.style.border = 'solid 1px #d5d5d5';
        ...
        shapeInfo.appendChild(shapeSelect);
    ...
    // 语义描述
    {
        ...
        concept.style.border = '2px solid #ebebeb';
        concept.style.padding = '1px';
        ...
        definitionContent.style.width = '90%';
        ...
        relateConceptContent.style.width = '90%';
        ...
        classContent.style.width = '95%';
        classContent.style.height = '40px';
        classContent.style.outline = 'none';
        classContent.style.padding = '6px';
        classContent.rows = 15;
    ...
    // 属性特征
    {
        ...
        property.style.border = '2px solid #ebebeb';
        property.style.padding = '1px';
        ...
        addEmptyPropertyFolder.style.width = "26px";
        addEmptyPropertyFolder.style.height = "26px";
        addEmptyPropertyFolder.style.padding = "0";
        ...
        addEmptyPropertyFolder.style.marginTop = '-14%';
        addEmptyPropertyFolder.style.marginRight = '20%';
        var that = this;
        addEmptyPropertyFolder.onclick = function () {
            if (propertySelect.selectedIndex > 0) {
                that.createLocalizationPalette(propertySelect.options[propertySelect.selectedIndex].text, propertyContentDiv);
            }
        }
        ...
        propertySelect.style.padding = '6px';
    ...
    // 演变过程
    {
        ...
        process.style.border = '2px solid #ebebeb';
        process.style.padding = '1px';
        ...
        var addProcess =  ()=> {
            ...
            processNameContent.style.padding = '6px';
            ...
            processDescTitle.innerHTML = "Desc: ";
            processDescTitle.style.margin = "3px 0 0 1px";
            var processDescContent = document.createElement("input");
            ...
            processDescContent.style.width = '60%';
            ...
            addEmptyProcessFolder.style.width = "26px";
            addEmptyProcessFolder.style.height = "26px";
            ...
            addEmptyProcessFolder.style.marginTop = '-14%';
            ...
            removeEmptyProcessFolder.style.width = "26px";
            removeEmptyProcessFolder.style.height = "26px";
            ...
            removeEmptyProcessFolder.style.marginTop = '-14%';
            removeEmptyProcessFolder.style.marginRight = '5%';

            var that = this;
            addEmptyProcessFolder.onclick = function () {
                if (processSelect.selectedIndex > 0) {
                    that.createLocalizationPalette(processSelect.options[processSelect.selectedIndex].text, processContentDiv);
                }
            }
            ...
            processSelect.style.padding = '6px';
            processSelect.style.display = 'block';
            processSelect.style.marginBottom = "10px";
    ...
    // 要素关系
    {
        ...
        var that = this;
        addEmptyElementFolder.onclick = function () {
            var title = [];
            ...
            selected = [];
            that.createLocalizationPalette(title.join(','), elementContentDiv);
        ...
        for (var i = 0; i < this.elementArray.length; i++) {
            var option = document.createElement('option');
            option.text = this.elementArray[i];
            option.style.fontFamily = 'Sans-serif';
    ...
    // 空间位置
    {
        ...
        spaceDetialTitle.innerHTML = "Relate Concepts: ";
...
ArrangePanel.prototype.addEnrichInfoDiv = function (div, title, textDiv) {
    ...
    textDiv.style.border = 'solid 1px #d5d5d5';
 ...
 *addEnrichTextArea
 */
ArrangePanel.prototype.addEnrichTextArea = function (div, title) {
    var titlePart = document.createElement('div');
    titlePart.innerHTML = title;
    var textarea = document.createElement('textarea');
    textarea.setAttribute('id', 'relations');
    textarea.setAttribute("type", "text");
    textarea.rows = 15;
    textarea.style.fontSize = '12px';
    textarea.style.fontFamily = 'Arial';
    ...
    // titlePart.appendChild(textarea);
    div.appendChild(titlePart);
    div.appendChild(textarea);
}
...
ArrangePanel.prototype.createLocalizationPalette = function (title, div) {
    ...
    Localizations.push(Localization);
    var that = this;
    div.appendChild(that.addPalette(Localizations[0],div.id));
}
...
ArrangePanel.prototype.addPalette = function (Localization,id) {
    var div = document.createElement("div");
...
//添加一个属性的选择框
// ArrangePanel.prototype.propertyArray = ['时间', '影响因素', '形状外观', '危害', '益处'];
ArrangePanel.prototype.propertyArray = ['物理属性', '化学属性', '生物属性', '人文属性', '经济属性','社会属性'];
ArrangePanel.prototype.processArray = ['发生条件', '影响因素', '发生时间', '精确信息', '模糊信息','参与要素'];
ArrangePanel.prototype.elementArray = ['风', '雨', '雷', '电', '金','木', '水','火'];
ArrangePanel.prototype.classificationArray = ["水系统", "土壤系统","大气系统","生态系统", "人文/社会/经济","陆地地表","海洋系统", "固体地球系统"];
ArrangePanel.prototype.shapeArray = ["多面体", "点状","线状","面状", "像素点"];
...
DiagramFormatPanel.prototype.init = function () {
  ...
  * */
    ...
    if (graph.isEnabled()) {
        ...
        // this.container.appendChild(this.addSearchInfo(this.createPanel()));
        /*
    lyc
    * 这里控制隐藏右侧展示的信息Option，paper以及style
    *
    * */
 ...
 * lyc edit
 */
DiagramFormatPanel.prototype.addSearchInfo = function (div) {
    var ui = this.editorUi;
    var editor = ui.editor;
    var graph = editor.graph;
