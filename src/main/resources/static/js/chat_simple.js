...
new Vue({
  ...
  methods: {
    ...
    * */
    applyRight(){
      this.socket.emit("applyRight",JSON.stringify(this.loginUser));
    },

    syncXml(){
      //同步画布、相关图片、规则、和选中状态
      var editorUi = parent.$('#ModelEditor')[0].contentWindow.editorUi;
      var newXml = editorUi.editor.getGraphXml().outerHTML;
      var relateImages = parent.$('#ModelEditor')[0].contentWindow.RelateImages;
      var geoRules = parent.$('#ModelEditor')[0].contentWindow.GeoRules;
      var selected = editorUi.editor.graph.getSelectionCell();
      var _this = this;

      if (selected != undefined){
        if (this.selected == selected.id && this.xml == newXml && this.relateImages == relateImages){
          return
        }
        this.selected = selected.id
        this.xml = newXml
        this.relateImages = relateImages
        this.socket.emit("syncXml",this.xml,selected.id,JSON.stringify(relateImages),JSON.stringify(geoRules));
        clearInterval(this.check)
        this.check = setInterval(()=>{
          _this.releaseRight()
        },10000)
      }else if (selected == undefined && JSON.stringify(this.relateImages) != JSON.stringify(relateImages)) {
        this.relateImages = relateImages
        this.socket.emit("syncXml",this.xml,-1,JSON.stringify(relateImages),JSON.stringify(geoRules));
      }else if (selected == undefined ){
        this.socket.emit("syncSelect",-1);
      }
    },

    releaseRight(){
      this.socket.emit("releaseRight");
    },
    /**/
    ...
    initSocketEvent() {
      ...
      //协同绘图相关回调
      _this.socket.on("changeRight",(user) => {
        this.currentControlUser = user;
        /**
         * 在这里设置 graph.setEnabled() 没有作用
         * 需要在modelEditor文件中进行设置
         * 原因可能是变量是按值传递
         */
        parent.$('#ModelEditor')[0].contentWindow.changeRight();
        console.log(user.name + " get right")
      })
      _this.socket.on("getRight",() => {
        this.currentControlUser = this.loginUser
        parent.$('#ModelEditor')[0].contentWindow.getRight();
        console.log(this.loginUser.name +" get right")

        //定时检查是否改动，释放控制权，或者，同步显示操作
        _this.sync = setInterval(()=>{
          _this.syncXml()
        },500)
        _this.check = setInterval(()=>{
          _this.releaseRight()
        },10000)

      })
      _this.socket.on("wait",(user) => {
        console.log("please wait, "+user.name+" is controlling.")
      })
      _this.socket.on("refreshXml",(xml,selected,relatedImages,geoRules) =>{
        parent.$('#ModelEditor')[0].contentWindow.refreshXml(xml,selected,relatedImages,geoRules);
      })
      _this.socket.on("release",(user) =>{
        this.currentControlUser = {};
        parent.$('#ModelEditor')[0].contentWindow.changeRight();
        console.log(user.name+" released control.")
        clearInterval(this.check)
        clearInterval(this.sync)
      })
      _this.socket.on("cellSelect",(selected)=>{
        parent.$('#ModelEditor')[0].contentWindow.cellSelect(selected);
      })
    },
    changeChannel(user) {
    ...
    login() {
      ...
      user.type = "user"
      user.draw = "true"
      var str = encodeURI(JSON.stringify(user), "utf-8");
  ...
  mounted(){
    //提升成 全局函数和变量
    window.applyRight = this.applyRight;
